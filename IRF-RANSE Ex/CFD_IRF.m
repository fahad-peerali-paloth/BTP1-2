...
omegaF = 2*pi()/TP ;
M = 1000*1.429; 
Ry=1.71; %radius of gyration from centroid
Iyy = M*(Ry^2); 
c33 = 53683; %rho*g*Awp;
c35=16752; %rho*g*first_moment_of_area
c53=16752; %rho*g*first_moment_of_area
c55 =161135; %M*g**GM_L
kf=1;
fHz=1/TP;
b_dist=[0.8 0.74 0.66 0.59 0.51 0.43 0.29 0.11 0.02 0.000001]; %breadth distribution at the Pressure view points ##PUT THE CURRECT VALUES
h_betwn_b=0.1428;
...
%      % fluent is called with f n closes
     ...
     delete('zerok.dat');
      ...
      P1(i) = csvread(fresult,5,1,[5,1,5,1]);
      P2(i) = csvread(fresult,13,1,[13,1,13,1]);
      P3(i) = csvread(fresult,21,1,[21,1,21,1]);
      P4(i) = csvread(fresult,29,1,[29,1,29,1]);
      P5(i) = csvread(fresult,37,1,[37,1,37,1]);
      P6(i) = csvread(fresult,45,1,[45,1,45,1]);
      P7(i) = csvread(fresult,53,1,[53,1,53,1]);
      P8(i) = csvread(fresult,61,1,[61,1,61,1]);
      P9(i) = csvread(fresult,69,1,[69,1,69,1]);
      P10(i) = csvread(fresult,77,1,[77,1,77,1]);
      P11(i) = csvread(fresult,85,1,[85,1,85,1]);
      P12(i) = csvread(fresult,93,1,[93,1,93,1]);
      P13(i) = csvread(fresult,101,1,[101,1,101,1]);
 ...
 % %  fclose(fresult);
     ...
     % cfdforce(i) = (1/0.2*L)*(P1(i)*bow_A(1)+P2(i)*bow_A(2)+P3(i)*bow_A(3)+P4(i)*bow_A(4)+P5(i)*bow_A(5)+P6(i)*bow_A(6)+P7(i)*bow_A(7)+P8(i)*bow_A(8)+P9(i)*bow_A(9));
       cfdforce(i) = (1/(0.2*L))*(P1(i)*bow_A(1)+P2(i)*bow_A(2)+P3(i)*bow_A(3)+P4(i)*bow_A(4)+P5(i)*bow_A(5)+P6(i)*bow_A(6)+P7(i)*bow_A(7)+P8(i)*bow_A(8));
 end   
 %%    
     %%%%%%%%%%
    ...
    b5 = F55(i)-c55*z5(i)+ A55*omegaF^2*z5(i)- Damp55+ A53*omegaF^2*z3(i) - del_t*Damp53-c53*z3(i)+(cfdforce(i)*(L*(0.3+0.4/3)))    - Crad55*z5(i) - Crad53*z3(i);
    ...
    b5 = F55(i+1)-c55*z5(i+1)+ A55*omegaF^2*z5(i+1) - Damp55+ A53*omegaF^2*z3(i) - del_t*Damp53-c53*z3(i)+(cfdforce(i)*(L*(0.3+0.4/3)))  - Crad55*z5(i+1) - Crad53*z3(i+1);
    v3(i+1) = v3(i) + del_t*b3/(M);%  ((z(i)-z(i-1))/del_t)+del_t*a;
    z3(i+1) = z3(i) + del_t*v3(i+1);
...
%%
    ...
    %     
        ...
        ta = ['a = ' num2str(a) ';\r\n'];
        tb = ['b = ' num2str(b) ';\r\n'];
        tc = ['w = 2*pi*' num2str(fHz) ';  /* 2Hz frequency */\r\n'];
